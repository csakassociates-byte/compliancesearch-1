/**
 * Board Report Rule 8 (Full) — docx generator
 * For Section 8 Companies, FPC, and non-small Private Limited
 */
...
export async function buildBoardReportRule8Docx(data: AnnualFilingData): Promise<Buffer> {
  ...
  const dirRows = data.directors
    ...
    .map((d, i) => {
      ...
      return [String(i + 1), d.name, d.din || "—", d.designation, d.category || "—", apptText, d.dateOfCessation ? fmtDate(d.dateOfCessation) : "—", statusLabel];
    });
  const dirTable = buildTable(
    ["Sl.", "Name of Director", "DIN", "Designation", "Category", "Date of Appointment", "Date of Cessation", "Status"],
    dirRows.length > 0 ? dirRows : [["—", "—", "—", "—", "—", "—", "—", "—"]],
    [400, 1800, 1100, 1500, 1000, 1300, 1300, 738],
    { fontSize: SZ9 }
  );

  const fyFromDate = new Date(`${fyStart}-04-01`);
  const fyToDate = new Date(`${Number(fyStart) + 1}-03-31`);
  const isInFY = (dateStr: string) => { if (!dateStr) return false; const d = new Date(dateStr); return d >= fyFromDate && d <= fyToDate; };
  ...
  if (appointed.length === 0 && resigned.length === 0) {
    dirChangesParas.push(p("There were no changes in the composition of the Board of Directors of the Company during the Financial Year under review."));
  } else {
    if (appointed.length > 0) {
      ...
      dirChangesParas.push(buildTable(
        ...
        [2800, 1200, 2200, 2638], { fontSize: SZ9 }
      ));
    }
    if (resigned.length > 0) {
      dirChangesParas.push(p(`During the Financial Year ${fy}, the following Director(s) resigned / ceased to hold office as Director(s) of the Company:`));
      dirChangesParas.push(buildTable(
  ...
  const children: (Paragraph | Table)[] = [
    ...
    new Paragraph({ children: [r("DIRECTORS' REPORT", { bold: true, size: SZ12, underline: true })], alignment: AlignmentType.CENTER, spacing: { after: 60 } }),
    ...
    pr([r(`The ${membersLabel},`, { size: SZ12 })], { align: AlignmentType.LEFT, after: 60 }),
    ...
    pr([
      ...
      r(` of the Company together with the Audited Financial Statements${isSection8 ? ` (${pandlLabel})` : ""} for the Financial Year ended 31st March, ${fyEnd}.`, { size: SZ12 }),
    ]),
    ...(isSection8 ? [p("The Company is a Company incorporated under Section 8 of the Companies Act, 2013 and is committed to its objects as stated in its Memorandum of Association. The Company is prohibited from distributing its profits as dividends to its members.")] : []),
    ...(isFPC ? [p("The Company is a Farmer Producer Company (FPC) registered under Part IXA of the Companies Act, 1956 as preserved by Section 465 of the Companies Act, 2013, formed to promote the economic interests of its Producer Members engaged in primary produce activities.")] : []),
    ...
    buildTable(
      ...
      [
        [isSection8 ? "Income from Operations" : "Revenue from Operations", fmtRs(data.financials.revenueFromOperations), fmtRs(data.financials.prevRevenueFromOperations)],
        ["Other Income", fmtRs(data.financials.otherIncome), fmtRs(data.financials.prevOtherIncome)],
        [isSection8 ? "Total Income" : "Total Income", fmtRs(data.financials.totalIncome), fmtRs(data.financials.prevTotalIncome)],
        ["Total Expenses", fmtRs(data.financials.totalExpenses), fmtRs(data.financials.prevTotalExpenses)],
        [isSection8 ? "Surplus / (Deficit) Before Tax" : "Profit / (Loss) Before Tax", fmtRs(data.financials.profitBeforeTax), fmtRs(data.financials.prevProfitBeforeTax)],
        ...
        [profitLabel, fmtRs(data.financials.profitAfterTax), fmtRs(data.financials.prevProfitAfterTax)],
        [isSection8 ? "Total Corpus / General Fund" : "Total Reserves & Surplus", fmtRs(data.financials.reservesAndSurplus), fmtRs(data.financials.prevReservesAndSurplus)],
      ],
      [5400, 2100, 2138], { fontSize: SZ10 }
    ),
    blankLine(),

    h2("2. Dividend"),
    p(isSection8
      ? `The Company is incorporated under Section 8 of the Companies Act, 2013 as a not-for-profit entity. The Company is prohibited from declaring or paying any dividend to its Members. The surplus, if any, arising from the ${pandlLabel} is applied solely towards the furtherance of the Company's objects as stated in its Memorandum of Association.`
      : isFPC
      ? (data.dividendDeclared
          ? `Your Directors are pleased to report that the Board has declared a Patronage Bonus / dividend of ${buildDividendText(data)} for the Financial Year ${fy} in compliance with the applicable provisions of the Companies Act.`
          : `Your Directors do not recommend any dividend on the Equity Shares of the Company for the Financial Year ${fy}. No Patronage Bonus has been declared for the Financial Year ${fy}.`)
      : (data.dividendDeclared
    ...
    p(isSection8
      ? `The surplus, if any, from the ${pandlLabel} for the Financial Year ${fy} has been retained in the corpus of the Company and applied towards its charitable objects. No amount has been transferred to any separate reserve during the year.`
      : `The Board of Directors has not proposed any amount to be transferred to reserves for the Financial Year ${fy}. The entire profit / (loss) has been retained in the ${pandlLabel} of the Company.`
    ),
    ...
    p(data.annualReturnWebLink
      ? `Pursuant to Section 92(3) of the Companies Act, 2013 read with Rule 12(1) of the Companies (Management and Administration) Rules, 2014, the Annual Return of the Company in Form MGT-7 for the Financial Year ended 31st March, ${fyEnd} is available on the website of the Company at: ${data.annualReturnWebLink}`
      : `Pursuant to Section 92(3) of the Companies Act, 2013 read with Rule 12(1) of the Companies (Management and Administration) Rules, 2014, the Annual Return of the Company in Form MGT-7 for the Financial Year ended 31st March, ${fyEnd} shall be filed with the Registrar of Companies. A copy of the Annual Return shall be made available at the Registered Office of the Company for inspection during business hours. The extract of Annual Return in Form MGT-9 is no longer required pursuant to the Companies (Amendment) Act, 2017.`
    ),

    h2("6. Number of Board Meetings"),
    pr([
      r(`During the Financial Year ${fy}, `, { size: SZ12 }),
    ...
    p(`The Annual General Meeting (AGM) of the Company for the Financial Year ${prevFY} was held on ${agmDate}${agmVenue}. The next AGM for the Financial Year ${fy} shall be held within the time stipulated under Section 96 of the Companies Act, 2013.`),

    h2("8. Share Capital"),
    p(`As on 31st March, ${fyEnd}, the Share Capital of the Company stood as under:`),
    buildTable(
      ["Particulars", "Amount (Rs.)"],
      [["Authorised Share Capital", fmtRs(data.financials.authorisedCapital)], ["Paid-up Share Capital", fmtRs(data.financials.paidUpCapital)]],
    ...
    p(`Pursuant to Section 134(3)(c) read with Section 134(5) of the Companies Act, 2013, your Directors confirm that:`),
    ...
    p(hasID
      ? "The Company has received necessary declarations from all Independent Directors of the Company pursuant to Section 149(7) of the Companies Act, 2013, confirming that they meet the criteria of independence as prescribed under Section 149(6) of the Companies Act, 2013. In the opinion of the Board, the Independent Directors possess the attributes of integrity, expertise and experience as required to be disclosed under Rule 8(5)(iiia) of the Companies (Accounts) Rules, 2014."
      : `As at 31st March, ${fyEnd}, the Company does not have any Independent Director on its Board. ${isSection8 ? "Section 8 Companies are generally not required to appoint Independent Directors unless they meet the thresholds prescribed under Rule 4 of the Companies (Appointment and Qualification of Directors) Rules, 2014." : "Accordingly, no declaration under Section 149(7) of the Companies Act, 2013 is required."}`
    ),
    ...
    p("The provisions relating to constitution of a Risk Management Committee under the SEBI (LODR) Regulations, 2015 are not applicable to the Company as it is an unlisted company. Accordingly, no Risk Management Committee has been constituted. However, the Board periodically reviews the risk landscape of the Company."),

    h2("17. Vigil Mechanism / Whistle-blower Policy"),
    p("The provisions of Section 177(9) and 177(10) of the Companies Act, 2013 regarding Vigil Mechanism are mandatory for listed companies, companies accepting deposits from the public, and companies with outstanding borrowings from banks exceeding Rs.50 Crores. The Company does not fall under any of the above categories. Accordingly, the Company is not required to establish a formal Vigil Mechanism / Whistle-blower Policy. The Board ensures a culture of transparency and integrity within the organization."),

    h2("18. Risk Management"),
    p(data.riskManagementDetails || "The Company has a risk management framework to identify, assess and mitigate the risks that could affect its operations and financial performance. The Board periodically reviews the risk landscape and takes appropriate steps to minimize and mitigate risks. The major risks identified include business risk, operational risk, financial risk and legal / regulatory risk. Adequate systems and processes are in place to manage these risks effectively."),

    h2("19. Conservation of Energy"),
    p(data.energyConservationDetails || "The operations of the Company are not energy-intensive. The Company has taken adequate measures for conservation of energy wherever possible."),
    p("(i) Steps taken or impact on conservation of energy: The Company continues to implement energy conservation practices such as usage of energy-efficient equipment and optimisation of energy usage in operations.\n(ii) Steps taken by the Company for utilising alternate sources of energy: Not applicable.\n(iii) Capital investment on energy conservation equipment: Nil."),

    h2("20. Technology Absorption"),
    p(data.technologyAbsorptionDetails || `The Company has not imported any technology during the Financial Year ${fy}. The Company has not incurred any expenditure on Research and Development. No technology absorption, adaptation or innovation was carried out during the year.`),
    ...
    p(data.hasRPT
      ...
      : `All related party transactions entered into during the Financial Year ${fy} were in the ordinary course of business and at arm's length basis. The Company has not entered into any contracts, arrangements or transactions with related parties which could be considered material. Accordingly, disclosure in Form AOC-2 is not required. Details of related party transactions as per AS-18 are set out in the Notes to the Financial Statements.`
    ),
    ...
    p(data.hasLoansGiven
      ...
      : `The Company has not given any loans, provided any guarantees or made any investments falling under the provisions of Section 186 of the Companies Act, 2013 during the Financial Year ${fy}.`
    ),
    ...
    p(data.hasDeposits
      ...
      : `The Company has not accepted any deposits from the public within the meaning of Sections 73 and 74 of the Companies Act, 2013 read with the Companies (Acceptance of Deposits) Rules, 2014 during the Financial Year ${fy}. No amount of principal or interest was outstanding as on 31st March, ${fyEnd}.`
    ),

    h2("26. Statutory Auditor"),
    p(getStatutoryAuditorText(data)),

    h2("27. Secretarial Audit"),
    p(`The provisions of Section 204 of the Companies Act, 2013 relating to Secretarial Audit are applicable to public companies with paid-up share capital of Rs.50 Crores or more, or turnover of Rs.250 Crores or more. The Company does not meet the aforesaid thresholds. Accordingly, Secretarial Audit is not applicable to the Company for the Financial Year ${fy}.`),

    h2("28. Cost Audit"),
    p(`The provisions of Section 148 of the Companies Act, 2013 relating to Cost Audit are not applicable to the Company for the Financial Year ${fy} as the Company is below the prescribed thresholds for maintenance of cost records and audit under the Companies (Cost Records and Audit) Rules, 2014.`),

    h2("29. Auditors' Report - Qualifications, Reservations and Adverse Remarks"),
    ...
    h2("30. Frauds Reported by Statutory Auditors"),
    ...
    h2("31. Internal Financial Controls"),
    ...
    h2("32. Material Changes and Commitments"),
    ...
    h2("33. Changes in Directors and Key Managerial Personnel"),
    ...dirChangesParas,

    h2("34. Significant and Material Orders by Regulators / Courts"),
    ...
    h2("35. Proceedings Under Insolvency and Bankruptcy Code, 2016"),
    p(`No application has been made and no proceeding is pending under the Insolvency and Bankruptcy Code, 2016 (31 of 2016) against the Company during the Financial Year ${fy}.`),

    h2("36. Details of Difference Between Amount of Valuation on One-Time Settlement"),
    ...
    h2("37. Particulars of Employees"),
    ...
    h2("38. Prevention of Sexual Harassment at Workplace (POSH)"),
    p("The Company has in place a policy on Prevention, Prohibition and Redressal of Sexual Harassment at the Workplace in accordance with the provisions of the Sexual Harassment of Women at Workplace (Prevention, Prohibition and Redressal) Act, 2013 and the Rules made thereunder."),
    buildTable(
      ["Particulars", "Number"],
      [
      ...
      [7500, 2138], { fontSize: SZ10 }
    ),
    blankLine(),

    h2("39. Compliance with Maternity Benefit Act, 1961"),
    ...
    h2(`40. Particulars of Employees (Gender-wise) as on 31st March, ${fyEnd}`),
    buildTable(
    ...
    h2("41. Compliance with Secretarial Standards"),
    p(`The Company has complied with the applicable Secretarial Standards issued by the Institute of Company Secretaries of India (ICSI), namely SS-1 (Secretarial Standard on Meetings of the Board of Directors) and SS-2 (Secretarial Standard on General Meetings), as applicable to the Company during the Financial Year ${fy}.`),

    h2("42. Business Responsibility and Sustainability Report"),
    p("The provisions of Regulation 34(2)(f) of the SEBI (Listing Obligations and Disclosure Requirements) Regulations, 2015 regarding Business Responsibility and Sustainability Report are applicable only to the top 1000 listed companies by market capitalisation. The Company is an unlisted company. Accordingly, this provision is not applicable to the Company."),

    h2("43. Corporate Governance"),
    p("The provisions relating to Corporate Governance Report as required under the SEBI (Listing Obligations and Disclosure Requirements) Regulations, 2015 are not applicable to the Company as it is an unlisted company. However, the Company strives to maintain good corporate governance practices in its day-to-day operations consistent with the principles of transparency, accountability and integrity."),

    h2("44. Acknowledgements"),
    p(`Your Directors would like to express their sincere appreciation for the assistance and co-operation received from the bankers, government authorities, regulatory authorities, customers and other business associates during the Financial Year ${fy}. Your Directors also wish to place on record their deep sense of appreciation to all the employees of the Company for their commendable teamwork and dedication.`),
    p(`The Board also thanks the ${membersLabel} of the Company for reposing confidence and trust in the management of the Company.`),

    new Paragraph({ children: [], pageBreakBefore: true, spacing: { after: 0, before: 0 } }),
    h2(`Composition of Board of Directors as on 31st March, ${fyEnd}`),
    dirTable,
    blankLine(),
