...
import {
  ...
  computeCertRanges,
  numberToWords as libNumberToWords,
  formatNum as libFormatNum,
  fmtDate,
  type CertShareholder,
  type CertSigner,
...
/* ── Local aliases for lib functions ────────────────────── */
const numberToWords = libNumberToWords;
const formatNum     = libFormatNum;

/* ── computeRanges: alias for lib version ───────────────── */
function computeRanges(shareholders: Shareholder[], startNo: number) {
  return computeCertRanges(shareholders, startNo);
}

const PRINT_CSS = ``;

/* ══════════════════════════════════════════════════════════════════
   HTML GENERATOR — delegates to shared lib
══════════════════════════════════════════════════════════════════ */
function generatePrintHTML(
  ...
  const company = {
    ...
    shareClass:  f.shareClass,
    nominalValue: f.nominalValue,
    paidUpValue:  f.paidUpValue,
    issueDate:    f.issueDate,
    issuePlace:   f.issuePlace,
  };
  const shareholders: CertShareholder[] = f.shareholders.map(sh => ({
    name:   sh.name,
    din:    sh.din,
    shares: sh.shares,
  }));
  const signers: CertSigner[] = f.signingDirectors
    .filter(d => d.name)
    .map(d => ({ name: d.name, designation: d.designation, din: d.din || undefined }));

  return generateShareCertificateHTML(company, shareholders, ranges, signers);
}
  ...
  async function saveShareholdersToDb(companyId: string) {
    ...
    for (let i = 0; i < f.shareholders.length; i++) {
      ...
      try {
        ...
        // Save shareholder record + certificate metadata
        await fetch('/api/shareholders', {
          ...
          body: JSON.stringify({
            ...
            // Certificate metadata — needed to reprint exact same format
            nominalValue: f.nominalValue || '10',
            paidUpValue: f.paidUpValue || '10',
            issuePlace: f.issuePlace || '',
            signingDirectorsJson: JSON.stringify(
              f.signingDirectors
                .filter(d => d.name)
                .map(d => ({ name: d.name, designation: d.designation, din: d.din || '' }))
            ),
          }),
        });
      } catch { /* silent */ }
