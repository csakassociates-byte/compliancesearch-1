...
export function buildAuditFooter(sealBase64?: string): Footer {
  ...
  if (sealBase64) {
    try {
      imgs.push(new ImageRun({ data: base64ToBuffer(sealBase64), transformation: { width: 90, height: 65 }, type: imgType(sealBase64) }));
    } catch {}
  }
  return new Footer({
    children: [
...
export function buildNotesFooter(
  ...
  for (const dir of (sigDirs ?? [])) {
    if (dir.base64) {
      try {
        imgs.push(new ImageRun({ data: base64ToBuffer(dir.base64), transformation: { width: 170, height: 65 }, type: imgType(dir.base64) }));
        imgs.push(r("     ", { size: SZ9 }));
      } catch {}
    }
  }
